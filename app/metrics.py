...
def calculate_metrics(df, compare_with):
    ...
    change_recommendation_percentage = this_recommendation_percentage - previous_metrics[0]
    change_average_money_value = this_average_money_value - previous_metrics[1]
    change_average_service_score = this_average_service_score - previous_metrics[2]
...
def display_metrics(df):
    ...
    st.write("""
        **Self-Sampling Bias:**
        ...
        """)
    compare_with = st.selectbox("Compare with:", ["previous month", "previous year"])
    ...
    with col4:
        st.metric(label="Total number of reviews", value=f"{review_count:.0f}")
    ...
    col1, space1, col2, space2, col3, space3, col4 = st.columns([1, 0.1, 1, 0.1, 1, 0.1, 1])
    with col1:
        st.metric(label="Previous Recommendation Percentage", value=f"{previous_recommendation_percentage:.2f}%")
    ...
    col1, space1, col2, space2, col3, space3, col4 = st.columns([1, 0.1, 1, 0.1, 1, 0.1, 1])
    with col1:
        st.metric(label="Recommendation Percentage", value=f"{this_recommendation_percentage:.2f}%", delta=f"{change_recommendation_percentage:.2f}% from last {compare_with}")
        st.caption('A higher percentage indicates customers are more likely to recommend.')
    with col2:
